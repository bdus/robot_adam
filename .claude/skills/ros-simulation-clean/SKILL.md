...
description: Aggressively clean up Gazebo/ROS2 simulation residual processes. This skill spawns a single background Bash sub-agent that runs the clean.sh script directly (no LLM involvement). The script kills gzserver/gzclient/ignition processes and robot_state_publisher/joint_state_publisher/spawn_entity (when simulation-related), then verifies via ros2 node list in a retry loop (max 5 attempts). Also detects and resolves port 11345 (Gazebo master) conflicts by extracting the blocking PID and force-killing it, plus cleans up Gazebo lock files (/tmp/*gazebo*, ~/.gazebo/*.lock). Use when Gazebo simulation leaves dangling nodes, port 1134 11345 is occupied ("bind: Address already in use"), simulation processes won't terminate cleanly, or duplicate joint_state_publisher/robot_state_publisher nodes accumulate.
user-invocable: true
---
...
- Duplicate `joint_state_publisher`/`robot_state_publisher` nodes — caused by repeated simulation launches without cleanup
